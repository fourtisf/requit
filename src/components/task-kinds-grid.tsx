...
export function TaskKindsGrid({ compact = false }: { compact?: boolean }) {
  return (
    <ul
      className={
        compact ? "grid gap-2.5 sm:grid-cols-2" : "grid gap-3 sm:grid-cols-2 lg:grid-cols-3"
      }
    >
      {TASK_KIND_ORDER.map((category) => {
        ...
        return (
          ...
          >
            ...
            <h3 className="mt-2 text-[16px] font-semibold tracking-[-0.025em]">{kind.title}</h3>
            <p className="mt-2 text-[13.5px] font-light leading-[1.65] text-fg-2">{kind.body}</p>
