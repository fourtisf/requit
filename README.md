...
| Seed cannot touch a live database | `prisma/seed.ts`, `npm run db:unseed` | Every offer in the seed is invented, and on the live site that is fake inventory on a page that says it has none. The seed refuses when the database holds accounts it did not create; `db:unseed` removes seed rows from one that already has them, in a transaction, and refuses outright if a reward, withdrawal or dispute exists against them. |
